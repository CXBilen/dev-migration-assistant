/**
 * Remediation: a structured "what to do about it" attached to attention items. v0.1 only displays
 * these (the app never executes anything); `command` is an argv array so a future automation step
 * can run it without a shell.
 */
import { z } from 'zod'
...
export const Remediation = z.object({
  ...
  /** Suggested command as argv (display only in v0.1). First element is the executable. */
...
export const REMEDIATIONS = {
  ...
  switchNode: (major: number): Remediation => ({
    ...
    detail: `The source machine used Node.js ${major}; pick the same major to avoid dependency surprises.`,
    command: ['nvm', 'install', String(major)],
  ...
  reinstallNativeDependencies: (packageManager: string | null): Remediation => ({
